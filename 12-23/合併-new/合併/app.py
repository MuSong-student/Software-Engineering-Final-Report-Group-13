...
from db import create_order,update_cart_items_order_id,finalize_order_in_db
...
def login():
    if request.method == 'POST':
        ...
        if user:
            ...
            if user.role == 2:
                return redirect(url_for('myMenus'))
            elif user.role == 1:
...
def update_order_id_route():
    ...
    try:
        ...
        # 新建訂單並取得 O_id
        order_id = create_order(user_id, restaurant_id, total_price)

        # 更新購物車項目的 O_id
        update_cart_items_order_id(user_id, restaurant_id, order_id)
...
def place_order_route():
    ...
    try:
        for restaurant_id, items in grouped_items.items():
            ...
            place_order_in_db(user_id, restaurant_id, total_price, quantities)
        flash("所有訂單已成功提交！")
        session['cart'] = []  # 清空購物車
        return redirect(url_for('customer_home'))
    except Exception as e:
