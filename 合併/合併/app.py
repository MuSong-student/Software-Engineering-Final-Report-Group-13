...
from databass import my_Menu,get_rid_by_account,add_menu_item,delete,get_menu_by_id,updategood,get_orders_by_restaurant, update_order_status, get_order_by_id, get_merchant_daily_stats
...
@app.route('/accept_order/<int:O_id>', methods=['POST'])
def accept_orders(O_id):
    update_order_status(O_id, 'accepted')
    return redirect('/orders')

@app.route('/reject_order/<int:O_id>', methods=['POST'])
def reject_orders(O_id):
    update_order_status(O_id, 'rejected')
    return redirect('/orders')
...
@app.route('/order/<int:O_id>', methods=['GET'])
def view_order_details(O_id):
